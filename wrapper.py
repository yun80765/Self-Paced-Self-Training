...
class Wrapper:
    ...
    def __init__(self, args):
        ...
        args.set_labeled_classes = [i for i in range(args.num_classes)]
        args.set_unlabeled_classes = [i for i in range(args.num_classes)]
        # args.num_pseudo_label = torch.ones(args.num_classes)*args.num_labeled
        args.num_pseudo_label = torch.empty(0,args.num_classes)
        args.np = []
        self.args = args
        self.model = None
        self.ema_model = None
    ...
    def create_network(self, ema=False):
        ...
        if self.args.arch in ['cnn13','WRN28_2','WRN28_8']:
    ...
    def prepare_datasets(self):
        ...
        print('args: ',self.args.dataset, self.args.train_data, self.args.train_data_noT, self.args.batch_size, self.args.n_cpus, self.args.num_labeled, self.args.num_valid_samples, self.args.seed, self.args.set_labeled_classes, self.args.set_unlabeled_classes)
    ...
    def train_cl(self):
        ...
        while True:
            ...
            # print('image_indices_hard_label: ', image_indices_hard_label)
            # labeled_counter = Counter(list(image_indices_hard_label.values()))
            # print('labeled_counter: ',labeled_counter)
            self.Coun_labeled(list(image_indices_hard_label.values()))
            self.vis()
            # reset network
            self.create_network()
    ...
    def Coun_labeled(self,labeled_list):
        """
        Counter number of labeled data/ pseudo labeled data per class
        """
        labeled_counter = list(Counter(labeled_list).values())
        print('label_counter: ',labeled_counter)
        self.args.num_pseudo_label = torch.vstack((self.args.num_pseudo_label,torch.tensor(labeled_counter).squeeze(0)))
        self.args.np.append(len(labeled_list))
        print('num of pseudo labels: ',self.args.np)

    def vis(self):
        plt.figure()
        x = self.args.num_pseudo_label.t()
        print(x)
        for i in range(self.args.num_classes):
            plt.plot(x[i],label=i)
        plt.legend(loc='lower right')
        plt.title('num labeled per class')
        plt.xlabel('epoch')
        plt.ylabel('num')
        plt.savefig(f'./figure/{self.args.add_name}_npc.png')
