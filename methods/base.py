...
class Train_Base():
    ...
    def validate(self, eval_loader, model, epoch, testing = False, use_zca = True, k=5):
        ...
        for i, (input, target) in enumerate(eval_loader):
            ...
            y_pred.extend(torch.max(output1, dim=-1)[1].cpu().tolist())
        ...
        # print('cf_mat.diagonal():',cf_mat.diagonal())
        # self.acc_per_class = torch.vstack((self.acc_per_class,torch.tensor(cf_mat.diagonal())))

        if testing == False:
            self.val_class_loss_list.append(meters['class_loss'].avg)
            self.val_error_list.append(meters['error1'].avg)

        return meters['top1'].avg, meters['top5'].avg, meters['class_loss'].avg, cf_mat.diagonal()
    ...
    def evaluate_after_train(self, modelName, validloader, testloader, model, optimizer, epoch):
        ...
        prec1, prec5, loss,_ = self.validate(validloader, model, epoch + 1)
        ...
        if self.args.debug:
            prec1_test, prec5_test, loss_test,_ = self.validate(testloader, model, epoch + 1)
